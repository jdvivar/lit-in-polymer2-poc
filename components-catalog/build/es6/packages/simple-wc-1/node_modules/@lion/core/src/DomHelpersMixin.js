 ...
 * @polymerMixin
 * @mixinFunction
 */export const DomHelpersMixin=dedupeMixin(superclass=>// eslint-disable-next-line
class DomHelpersMixin extends superclass{constructor(){super();this.__domHelpersCache=generateEmptyCache()}/**
       * To access an element with the id 'foo' in shadow DOM
       *
       * @param {number} id
       * @returns {*|undefined}
       */$id(id){let element=this.__domHelpersCache.$id[id];if(!element){element=this.shadowRoot.getElementById(id);this.__domHelpersCache.$id[id]=element}return element||void 0}/**
       * Provides access to the named slot node in shadow DOM for this name
       *
       * @param {string} name
       * @returns {*|undefined}
       */$name(name){let element=this.__domHelpersCache.$name[name];if(!element){element=this.shadowRoot.querySelector(`[name="${name}"]`);this.__domHelpersCache.$name[name]=element}return element||void 0}/**
       ...
       * @param {number} id
       * @returns {*|undefined}
       */$$id(id){let element=this.__domHelpersCache.$$id[id];if(!element){element=this.querySelector(`#${id}`);this.__domHelpersCache.$$id[id]=element}return element||void 0}/**
       ...
       * @param {string} slot
       * @returns {*|undefined}
       */$$slot(slot){let element=this.__domHelpersCache.$$slot[slot];if(!element){element=Array.from(this.children).find(child=>child.slot===slot);this.__domHelpersCache.$$slot[slot]=element}return element||void 0}/**
       * Remove from cache (individually or completely) via _clearDomCache()
       *
       * @param {string} type
       * @param {number} id
       * @private
       */_clearDomCache(type,id){if(type){this.__domHelpersCache[type][id]=void 0}else{this.__domHelpersCache=generateEmptyCache()}}});